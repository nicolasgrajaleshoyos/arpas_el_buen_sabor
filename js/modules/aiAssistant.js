...
const AIAssistant = {
    ...
    renderMessages() {
        ...
        container.innerHTML = this.messages.map(msg => `
            <div class="flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}">
                <div class="max-w-[80%] ${msg.role === 'user' ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-900'} rounded-lg px-4 py-2">
                    <p class="text-sm">${msg.content}</p>
                    <p class="text-xs mt-1 ${msg.role === 'user' ? 'text-emerald-100' : 'text-gray-500'}">
    ...
    renderWidget() {
        ...
        container.innerHTML = `
            <!-- Chat Window -->
            <div id="ai-chat-window" class="hidden fixed bottom-24 right-6 w-96 h-[500px] bg-white rounded-xl shadow-2xl flex flex-col z-40">
                <!-- Header -->
    ...
    showApiKeyModal() {
        ...
        modal.innerHTML = `
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
                <h3 class="text-xl font-bold text-gray-900 mb-4">Configurar API Key de Gemini</h3>
                <p class="text-sm text-gray-600 mb-4">Para usar el asistente de IA, necesitas una API Key de Google Gemini.</p>
                ...
                <form id="api-key-form" class="space-y-4">
                    ...
                    <div class="flex gap-3">
                        <button type="button" onclick="this.closest('.fixed').remove()" class="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium">
                            Cancelar
                        </button>
